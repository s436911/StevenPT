...
public class UI_ScoreManager : MonoBehaviour
{
	...
	public void Victory() {
		...
		diffText[0].text = "x" + (1 + (0.2f * SYS_StarmapManager.Direct.difficult)).ToString("f1");
		diffText[1].text = "x" + (1 + (0.2f * SYS_StarmapManager.Direct.difficult)).ToString("f1");
		diffText[2].text = "x" + (1 + (0.2f * SYS_StarmapManager.Direct.difficult)).ToString("f1");
		diffText[3].text = "x" + (1 + (0.2f * SYS_StarmapManager.Direct.difficult)).ToString("f1");

		getText[0].text = "+" + Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(0) * getRate[0] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))).ToString();
		getText[1].text = "+" + Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(2) * getRate[1] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))).ToString();
		getText[2].text = "+" + Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(3) * getRate[2] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))).ToString();
		getText[3].text = "+" + Mathf.RoundToInt(getCoin * getRate[3] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))).ToString();

		SYS_ResourseManager.Direct.ModifyResourceHome(0, Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(0) * getRate[0] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))));
		SYS_ResourseManager.Direct.ModifyResourceHome(2, Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(2) * getRate[1] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))));
		SYS_ResourseManager.Direct.ModifyResourceHome(3, Mathf.RoundToInt(SYS_ResourseManager.Direct.GetResource(3) * getRate[2] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))));
		SYS_ResourseManager.Direct.ModifyResourceHome(4, Mathf.RoundToInt(getCoin * getRate[3] * (1 + (0.2f * SYS_StarmapManager.Direct.difficult))));
